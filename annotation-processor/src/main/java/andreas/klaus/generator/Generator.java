...
public class Generator extends AbstractProcessor {
	...
	// private Map<String, VariableElement> fields = new HashMap<String,
	// VariableElement>();
	// private Map<String, ExecutableElement> methods = new HashMap<String,
	// ExecutableElement>();
		...
		for (Element elem : roundEnv.getElementsAnnotatedWith(ValidationDefClass.class)) {
			...
			for (Element child : children) {
				...
				if (a != null) {
					...
					ModelValidationFieldData mvfd = new ModelValidationFieldData(getParameterType(varEl),
							varEl.getSimpleName().toString(), a.notEmpty(), a.shortDateFormat(), a.integer(),
							a.currency(), a.gebueHCat(), a.length());
					note(mvfd.toString(), varEl);
					modelFieldMap.put(mvfd.getName(), mvfd);
				}
			}
			ModelValidationDefClassData classData = new ModelValidationDefClassData(this.packageName, this.className,
					modelFieldMap);
	...
	private void generateJGM(ModelClassData model, Element curClassElement) {
		try {
			...
			JavaFileObject jfo = processingEnv.getFiler()
					.createSourceFile(packageName + ".jgmodel." + className + "JGM");
			...
			e.printStackTrace();
			note("Error Generating File " + model.getJGMClassName() + ":" + e.getClass().getName() + " Message:"
					+ e.getMessage(), curClassElement);
		}
	...
	private void generateValidator(ModelValidationDefClassData model, Element curClassElement) {
		try {
			...
			JavaFileObject jfo = processingEnv.getFiler()
					.createSourceFile(packageName + "." + model.getValidatorClassname());
			...
			e.printStackTrace();
			note("Error Generating File " + model.getValidatorClassname() + ": " + e.getClass().getName() + " Message:"
					+ e.getMessage(), curClassElement);
		}
